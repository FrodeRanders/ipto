...
\begin{lyingfigure}
    \centering
    \includegraphics[width=.98\linewidth]{images/repo-schema.png}
    \captionof{figure}{Repository schema overview derived from the Mermaid ER model in \texttt{doc/Model.md}}
    \label{fig:RepositorySchemaOverview}
\end{lyingfigure}
\FloatBarrier

Figure~\ref{fig:RepositorySchemaOverview} shows the full relational layout at schema level. The remainder of this chapter walks the same structure in conceptual groups so that the storage model is easier to understand than a table-by-table listing alone.
...
This split is important. It allows the system to keep a durable identity and current status in the kernel while storing historical changes in the version table. The latest version is found by following \literalbreak{repo\_unit\_kernel.lastver} to the matching row in \texttt{repo\_unit\_version}. Java repository queries use this directly when loading either a specific version or the latest one.
...
The database distinguishes between \emph{what an attribute means} and \emph{where an attribute value occurs}, analogous to, but simpler than, ISO/IEC 11179.
