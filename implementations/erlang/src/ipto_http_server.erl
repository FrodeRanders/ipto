...
-type http_reply() :: ok | {error, ipto_reason()}.
...
http_enabled() ->
    case os:getenv("IPTO_HTTP_ENABLED") of
        false ->
            application:get_env(ipto, http_enabled, false);
        Value ->
            to_bool(Value, false)
    end.
...
http_port() ->
    case os:getenv("IPTO_HTTP_PORT") of
        false ->
            application:get_env(ipto, http_port, 8080);
        Value ->
