...
store_unit_json(UnitMap) ->
    ipto_db:memory_store_unit_json_backend(UnitMap).
...
search_units(Expression, Order, PagingOrLimit) ->
    ipto_db:memory_search_units_backend(Expression, Order, PagingOrLimit).
...
add_relation(UnitRef, RelType, OtherUnitRef) ->
    ipto_db:memory_add_relation_backend(UnitRef, RelType, OtherUnitRef).
...
add_association(UnitRef, AssocType, RefString) ->
    ipto_db:memory_add_association_backend(UnitRef, AssocType, RefString).
...
remove_association(UnitRef, AssocType, RefString) ->
    ipto_db:memory_remove_association_backend(UnitRef, AssocType, RefString).
...
get_right_association(UnitRef, AssocType) ->
    ipto_db:memory_get_right_association_backend(UnitRef, AssocType).
...
get_right_associations(UnitRef, AssocType) ->
    ipto_db:memory_get_right_associations_backend(UnitRef, AssocType).

-spec get_left_associations(association_type(), ref_string()) -> ipto_result([association()]).
get_left_associations(AssocType, RefString) ->
    ipto_db:memory_get_left_associations_backend(AssocType, RefString).
...
count_right_associations(UnitRef, AssocType) ->
    ipto_db:memory_count_right_associations_backend(UnitRef, AssocType).
...
count_left_associations(AssocType, RefString) ->
    ipto_db:memory_count_left_associations_backend(AssocType, RefString).

-spec lock_unit(unit_ref_value(), lock_type(), ref_string()) -> ok | already_locked | {error, ipto_reason()}.
lock_unit(UnitRef, LockType, Purpose) ->
    ipto_db:memory_lock_unit_backend(UnitRef, LockType, Purpose).
...
create_attribute(Alias, Name, QualName, Type, IsArray) ->
    ipto_db:memory_create_attribute_backend(Alias, Name, QualName, Type, IsArray).

-spec get_attribute_info(name_or_id()) -> {ok, attribute_info()} | not_found | {error, ipto_reason()}.
get_attribute_info(NameOrId) ->
    ipto_db:memory_get_attribute_info_backend(NameOrId).
