# ipto Hot Upgrade Guide

This document describes a practical procedure for upgrading a running `ipto` node with OTP release handling.

## 1) What is in place

- `rebar.config` now contains a `relx` release definition for `ipto` version `0.2.0`.
- `src/ipto.appup` contains upgrade/downgrade instructions between `0.1.0` and `0.2.0`.
- HTTP lifecycle is handled by supervised worker `ipto_http_server` (child of `ipto_sup`), which is safer during upgrade than starting listeners directly in `application:start/2`.
...
- Each new release version should include:
  - updated `vsn` in `src/ipto.app.src`
  - updated `relx` release version in `rebar.config`
  - updated `src/ipto.appup` entries for previous version(s)
...
```sh
rebar3 relup --upfrom 0.1.0 --output-dir _build/default/rel/ipto
```
...
```erlang
{ok, Vsn} = release_handler:unpack_release("ipto-0.2.0").
...
`src/ipto.appup` includes downgrade instructions from `0.2.0` to `0.1.0`.

## 6) HTTP behavior during upgrade

`ipto_http_server` ensures HTTP listener state converges to config both at startup and code change:
...
- Env vars: `IPTO_HTTP_ENABLED`, `IPTO_HTTP_PORT`
...
1. Copy previous `src/ipto.appup` and change top version string to `X.Y.Z`.
