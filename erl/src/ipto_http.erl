...
start() ->
    Port = env_int("IPTO_HTTP_PORT", 8080),
    start(#{port => Port}).
    ...
    case code:ensure_loaded(cowboy) of
        {module, cowboy} ->
            ...
            Dispatch = cowboy_router:compile([
                {'_', [
                    {"/graphql", ipto_http_graphql_handler, #{}},
                    {"/health", ipto_http_health_handler, #{}}
                ]}
            ]),
            TransOpts = [{ip, {0, 0, 0, 0}}, {port, Port}],
            ProtoOpts = #{env => #{dispatch => Dispatch}},
            case cowboy:start_clear(ipto_http_listener, TransOpts, ProtoOpts) of
...
stop() ->
    case code:ensure_loaded(cowboy) of
        {module, cowboy} ->
            cowboy:stop_listener(ipto_http_listener);
        _ ->
            ok
    end.
