...
-spec execute(binary() | string(), map(), map()) -> erepo_result(map()).
...
reload_schema() ->
    ok = erepo_graphql_config:reload(),
...
ensure_schema_loaded() ->
    SchemaData = erepo_graphql_config:get_schema(),
    Mapping = erepo_graphql_config:get_mapping(),
