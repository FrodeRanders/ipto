...
pg_backend_roundtrip() ->
    ...
    case erepo:get_tenant_info(1) of
        ...
        {ok, _Tenant} ->
            ...
            {ok, RightRel} = erepo:get_right_relation(UnitRef, 1),
            {ok, RightRels} = erepo:get_right_relations(UnitRef, 1),
            {ok, LeftRels} = erepo:get_left_relations(OtherRef, 1),
            {ok, 1} = erepo:count_right_relations(UnitRef, 1),
            {ok, 1} = erepo:count_left_relations(OtherRef, 1),
            ...
            ok = erepo:add_association(OtherRef, 2, <<"case:pg-test">>),
            {ok, RightAssoc} = erepo:get_right_association(UnitRef, 2),
            {ok, RightAssocs} = erepo:get_right_associations(UnitRef, 2),
            {ok, LeftAssocs} = erepo:get_left_associations(2, <<"case:pg-test">>),
            {ok, 1} = erepo:count_right_associations(UnitRef, 2),
            {ok, 2} = erepo:count_left_associations(2, <<"case:pg-test">>),
            ...
            ok = erepo:remove_association(OtherRef, 2, <<"case:pg-test">>),
            ok
    end.
