...
neo4j_backend_roundtrip() ->
    ...
    {ok, RightRel} = erepo:get_right_relation(ARef, 1),
    {ok, RightRels} = erepo:get_right_relations(ARef, 1),
    ...
    ok = erepo:add_association(BRef, 2, <<"neo4j:assoc">>),
    {ok, RightAssoc} = erepo:get_right_association(ARef, 2),
    {ok, RightAssocs} = erepo:get_right_associations(ARef, 2),
    {ok, LeftAssocs} = erepo:get_left_associations(2, <<"neo4j:assoc">>),
    {ok, 1} = erepo:count_right_associations(ARef, 2),
    {ok, 2} = erepo:count_left_associations(2, <<"neo4j:assoc">>),
    ...
    ok = erepo:remove_association(BRef, 2, <<"neo4j:assoc">>),
